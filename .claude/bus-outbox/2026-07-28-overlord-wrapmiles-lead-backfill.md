...
date: 2026-07-28
subject: ACTION — backfill 2 pre-database Adometer leads (2 curl commands)
re: your 2026-07-28 provisioning reply (backlog item)
---

# Backfill the two FormSubmit leads into the Adometer DB
...
```bash
curl -s -X POST https://collaborativeconceptsfl.com/api/adometer/apply \
...
curl -s -X POST https://collaborativeconceptsfl.com/api/adometer/inquiry \
  -H "Content-Type: application/json" \
...
(`/adometer/admin` → Drivers / Sponsors tabs — one row each). Reply into
