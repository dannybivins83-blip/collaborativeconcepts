...
- **Adometr** at `/adometr/` — car-wrap sponsorship marketplace (pay-per-mile
  mobile OOH). Landing page + three portals: `/adometr/admin/` (matchmaking
  desk), `/adometr/driver/`, `/adometr/sponsor/`. API in `api/adometr.py`
  (`/api/adometr/*`), registered from `api/index.py` like permits. Storage is
  ...
  `ref_code` (NAME-XXXX); `/adometr?ref=CODE` persists to localStorage and
  prefills `referred_by` on both forms; driver portal has a share card
  (copy/native share/WhatsApp/SMS/QR via `/api/adometr/qr?ref=`) with live
