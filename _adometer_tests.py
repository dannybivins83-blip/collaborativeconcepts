"""Offline tests for the Adometer portals API (api/adometer.py).
...
ledger → driver + sponsor portal payloads.  Run:  python3 _adometer_tests.py
"""
...
r = client.post("/api/adometer/apply", json={"email": "pete@example.com",
                                              "name": "Pete Updated", "vehicle": "2019 F-150"})
check("duplicate apply upserts", r.status_code == 200, r.data)
r = client.post("/api/adometer/apply", json={"email": "not-an-email"})
check("bad email rejected", r.status_code == 400)
r = client.post("/api/adometer/apply", json={"email": "bot@x.com", "_honey": "gotcha"})
check("honeypot swallowed", r.status_code == 200)

r = client.post("/api/adometer/inquiry", json={
...
print("== admin auth ==")
r = client.post("/api/adometer/admin/login", json={"key": "wrong"})
check("wrong admin key 401", r.status_code == 401)
r = client.post("/api/adometer/admin/login", json={"key": "test-admin-key"})
check("admin login 200", r.status_code == 200)
admin = j(r)["token"]
r = client.get("/api/adometer/admin/drivers")
check("no token 401", r.status_code == 401)
r = client.get("/api/adometer/admin/drivers", headers=auth("garbage.token"))
check("bad token 401", r.status_code == 401)

print("== admin triage ==")
r = client.get("/api/adometer/admin/drivers", headers=auth(admin))
drivers = j(r)["drivers"]
...
did = drivers[0]["id"]
r = client.patch(f"/api/adometer/admin/drivers/{did}", headers=auth(admin),
...
r = client.get("/api/adometer/admin/sponsors", headers=auth(admin))
sid = j(r)["sponsors"][0]["id"]
r = client.patch(f"/api/adometer/admin/sponsors/{sid}", headers=auth(admin),
...
r = client.post("/api/adometer/admin/campaigns", headers=auth(admin), json={
...
r = client.get("/api/adometer/admin/campaigns", headers=auth(admin))
camp = j(r)["campaigns"][0]
check("rate stored in cents", camp["rate_cents_per_mile"] == 22)
cid = camp["id"]
r = client.post("/api/adometer/admin/matches", headers=auth(admin),
...
r = client.patch(f"/api/adometer/admin/matches/{mid}", headers=auth(admin),
                 json={"status": "active", "wrapped_at": "2026-08-05"})
check("match activated", r.status_code == 200)
r = client.patch(f"/api/adometer/admin/campaigns/{cid}", headers=auth(admin),
                 json={"status": "active"})
check("campaign activated", r.status_code == 200)

print("== driver portal ==")
r = client.post("/api/adometer/login", json={"role": "driver",
...
dtok = j(r)["token"]
r = client.get("/api/adometer/driver/me", headers=auth(dtok))
me = j(r)
...
r = client.post("/api/adometer/driver/mileage", headers=auth(dtok), json={
...
r = client.post("/api/adometer/driver/mileage", headers=auth(admin), json={
    "match_id": match_id, "period": "2026-08", "miles": 100})
check("admin token not driver 401", r.status_code == 401)

print("== approval + money ==")
r = client.get("/api/adometer/admin/mileage", headers=auth(admin))
gid = j(r)["mileage"][0]["id"]
r = client.patch(f"/api/adometer/admin/mileage/{gid}", headers=auth(admin),
                 json={"status": "approved", "miles_approved": 1800})
check("mileage approved", r.status_code == 200)
r = client.get("/api/adometer/admin/ledger", headers=auth(admin))
ledger = j(r)["ledger"]
...
client.patch(f"/api/adometer/admin/mileage/{gid2}", headers=auth(admin),
...
r = client.post("/api/adometer/login", json={"role": "sponsor",
                                              "email": "ray@example.com", "code": sponsor_code})
check("sponsor login", r.status_code == 200, r.data)
stok = j(r)["token"]
r = client.get("/api/adometer/sponsor/me", headers=auth(stok))
sp = j(r)
...
r = client.get("/api/adometer/sponsor/me", headers=auth(dtok))
...
check("ref code generated", bool(ref.get("code")) and ref["code"].startswith("POOL"),
      ref.get("code"))
check("ref link built", ref.get("link", "").endswith("/adometer?ref=" + ref.get("code", "")))
check("qr url present", ref.get("qr_url", "").startswith("/api/adometer/qr"))
...
r = client.get("/api/adometer/qr?ref=" + ref["code"])
check("qr endpoint", r.status_code == 200 and r.data[:4] == b"\x89PNG", r.status_code)
r = client.get("/api/adometer/qr?ref=<script>")
...
r = client.post("/api/adometer/driver/photos", headers=auth(dtok),
...
pid = photos[0]["id"]
r = client.get(f"/api/adometer/photos/{pid}", headers=auth(dtok))
check("owner fetches photo", r.status_code == 200 and r.data[:4] == b"\x89PNG")
r = client.get(f"/api/adometer/photos/{pid}?t={admin}")
check("admin fetches via ?t=", r.status_code == 200)
r = client.get(f"/api/adometer/photos/{pid}")
check("anon photo denied", r.status_code == 401)
r = client.post("/api/adometer/login", json={"role": "driver",
                                              "email": "rita@example.com", "code": "X"})
r = client.get("/api/adometer/admin/photos?driver_id=" + str(did), headers=auth(admin))
